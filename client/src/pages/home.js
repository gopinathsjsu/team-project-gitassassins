...
class home extends Component {
	componentDidMount() {
		...
		axios.get("hotel/getAll").then((res) => {
	...
	displayHotels() {
		if (this.props.hotel.allHotels.length > 0) {
			console.log(
				"display restaurants" + JSON.stringify(this.props.user.location)
			);
			...
			let hotels =
				this.props.user.location === ""
					...
					: allHotels.filter((hotel) => {
							return hotel.hotelAddress.city
								.toLowerCase()
								.includes(
									this.props.user.location.toLowerCase()
								);
					  });
